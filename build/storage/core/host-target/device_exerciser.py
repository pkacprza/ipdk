...
from sma_handle import SmaHandle
    ...
    def plug_device(self, device_handle: str) -> None:
        ...
        try:
            ...
            if not device.is_plugged():
                logging.info("Device is not plugged. Start unplug")
                device.plug()
                logging.info("End plug")
            else:
                logging.info(f"Device {device_handle} is already plugged")
        except (DriverError, DeviceError) as ex:
